...
2. System shall check if provided email exist in adminUser table, if not system shall output `Unauthorized error` error
3. If email exist, system shall check if password match between input password and stored password. 
4. System shall hash the input password and compare it to the stored hash, but not in any way edit the password
